...
class MultimodalExtremaSearch:
    num_batch: int = 1
    global_bounds: torch.Tensor = torch.tensor([[0.0, 0.0], [1.0, 1.0]], dtype=dtype)
    ...
    global_state: NdGlobalSearchState = None
    iteration_num: int = 0
    ...
    def initialize_global_state(self, x_init, y_init):
        ...
        self.global_state = NdGlobalSearchState(x_global=x_init, y_global=y_init)
        # set up graph tracking subdomains of search space
        self.global_state.partition_graph = nx.DiGraph()
        initial_local_data = NdLocalSearchState(input_dim=self.input_dim, local_bounds=self.global_bounds,
                                                x_local=self.global_state.x_global,
                                                y_local=self.global_state.y_global,)
        initial_local_search = NdLocalExtremeSearch(self.max_local_evals, self.min_local_init_evals,
                                                    initial_local_data, self.input_dim,
        ...
        # init_extreme_y = self.global_state.y_global.max()
        # extreme_idx = self.global_state.y_global.argmax()
        init_extreme_x = self.global_state.x_global[extreme_idx[0]]
        # init_extreme_x = self.global_state.x_global[extreme_idx]
    ...
    def run_global_tead_exploration_evaluations(self):
        ...
        while tead_iter < self.num_tead_explore_evals:
            ...
            if self.tead_model_type == 'global':
                x_candidates, x_tead_scores = nglobal_tead(self.global_state.global_model, get_all_candidates=True)
            elif self.tead_model_type == 'piecewise':
                x_candidates, x_tead_scores = npiecewise_tead(self.global_state.x_global, self.global_state.y_global,
                                                              self.global_state.partition_graph,
    ...
    def compute_subdomain_preselect_scores(self):
        ...
        for n in graph_leaves:
            ...
            # #
            # new
            # get index of points inside local bounds of leaf node
            idx_ub = torch.where(x_cands < current_bounds[1], True, False)
            idx_lb = torch.where(x_cands >= current_bounds[0], True, False)
    ...
    def determine_penalty_radius(self, extrema_set):
        ...
        if len(extrema_set) <= 1:
            ...
            # n-dim mod - NOTE THAT THIS IS NOT TO NEAREST NEIGHBORS - SO MAY NEED TO BE CHANGED TO THAT
            dists = []
            for i in range(len(extrema_set)):
                for j in range(len(extrema_set)):
                    dists.append(torch.linalg.norm(extrema_set[i][0] - extrema_set[i][1]))
            #
    ...
    def get_penalty(self, x, score):
        ...
        if past_extrema is not None:
            ...
            for extrema in penalized_extrema:
                ...
                # new
                combined_bounds = torch.stack([lower_bound, upper_bound], dim=0)
                #
                ...
                # #
                # new
                idx_ub = torch.where(x < combined_bounds[1], True, False)
                idx_lb = torch.where(x >= combined_bounds[0], True, False)
                idx_both = idx_lb & idx_ub
                idx_select = torch.all(idx_both, dim=1)
                penalty[idx_select] *= 0.5
                #
            return penalty
        else:
    ...
    def evaluate_objective_at_split(self, node, l_bound: torch.Tensor, r_bound: torch.Tensor):
        ...
        # get indices for each node
        # left
        idx_left_ub = torch.where(initial_x_cands < l_bound[1], True, False)
        idx_left_lb = torch.where(initial_x_cands >= l_bound[0], True, False)
        idx_left = idx_left_lb & idx_left_ub
        # right
        idx_right_ub = torch.where(initial_x_cands < r_bound[1], True, False)
        idx_right_lb = torch.where(initial_x_cands >= r_bound[0], True, False)
        idx_right = idx_right_lb & idx_right_ub
        # get partitioned candidates and scores
        x_cands_left = initial_x_cands[torch.all(idx_left, dim=1), :]
        scores_left = initial_scores[torch.all(idx_left, dim=1)]
        x_cands_right = initial_x_cands[torch.all(idx_right, dim=1), :]
        scores_right = initial_scores[torch.all(idx_right, dim=1)]
        #
    ...
    def update_extreme(self):
        ...
        if new_extreme_y > current_extreme_y:  # assume 1-dim output y
            self.global_state.x_global_extreme = new_extreme_x
            self.global_state.y_global_extreme = new_extreme_y

    def get_new_search_state(self, node_bound: torch.Tensor):
        ...
        # #
        # new
        idx_ub = torch.where(x_node < node_bound[1], True, False)
        idx_lb = torch.where(x_node >= node_bound[0], True, False)
        idx_both = idx_lb & idx_ub
        idx_select = torch.all(idx_both, dim=1)
        x_node_new = x_node[idx_select, :]
        y_node_new = self.global_state.y_global[idx_select]  # may need to unsqueeze??
        #
        node_local_data = NdLocalSearchState(input_dim=self.input_dim, local_bounds=node_bound,
                                             x_local=x_node_new,
                                             y_local=y_node_new, )
    ...
    def run_global_partition_step(self):
        ...
        # split bounds to get bounds for new nodes
        # # old
        ...
        # new
        prior_bounds = node_to_split['data'].local_bounds
        # pick a random input dimension to split - could be done by Sobol or other criteria instead
        split_dim = torch.randint(0, self.input_dim, (1,))
        # calculate the center - i.e., split the bounds in the select dim just in half - could use other distance
        center_bound = (prior_bounds[1, split_dim] - prior_bounds[0, split_dim]) / 2.0 + prior_bounds[0, split_dim]
        # construct new bound objects
        left_bound = prior_bounds.clone()
        left_bound[1, split_dim] = center_bound
        right_bound = prior_bounds.clone()
        right_bound[0, split_dim] = center_bound
        #
        print("PARTITION: Splitting node ", self.current_max_preselect_node, " from ", prior_bounds, " to ",
              left_bound, " and ", right_bound)
        # set up new nodes
        left_local_data = self.get_new_search_state(left_bound)
        right_local_data = self.get_new_search_state(right_bound)
    ...
    def compute_global_select_scores(self):
        ...
        for n in graph_leaves:
            ...
            # #
            # new
            idx_ub = torch.where(x_candidates < current_bounds[1], True, False)
            idx_lb = torch.where(x_candidates >= current_bounds[0], True, False)
            idx_both = idx_lb & idx_ub
            idx_select = torch.all(idx_both, dim=1)
            current_cands = x_candidates[idx_select, :]  # unsqueeze(-1)?
            current_scores = x_tead_scores[idx_select]  # unsqueeze(-1)?
            #
            ...
            print("SELECT: For node ", n, " best x: ", cur_max_x, " best tead: ", cur_max_tead.item())
    ...
    def reset_leaf_node_local_data(self):
        ...
        for n in graph_leaves:
            ...
            # #
            # new
            idx_ub = torch.where(all_x < current_bounds[1], True, False)
            idx_lb = torch.where(all_x >= current_bounds[0], True, False)
            idx_both = idx_lb & idx_ub
            idx_select = torch.all(idx_both, dim=1)
            local_x = all_x[idx_select]
            local_y = all_y[idx_select]
            current_state.x_local = local_x  #.unsqueeze(-1)
            current_state.y_local = local_y  #.unsqueeze(-1)
    ...
    def run_selected_local_search(self, node):
        ...
        num_initial_tead = 1
        local_pre_search = NdLocalExtremeSearch(num_initial_tead, self.min_local_init_evals,
                                                current_local_data, dim=self.input_dim,
                                                objective_function=self.obj_func)
        ...
        print("LOCALSEARCH: Completed ", int(x_new.shape[0]), " new tead evaluations ")
        self.global_state.x_global = torch.cat((self.global_state.x_global, x_new), 0)
        self.global_state.y_global = torch.cat((self.global_state.y_global, y_new), 0)
        # run turbo points
        local_search = NdLocalExtremeSearch(self.max_local_evals, self.min_local_init_evals,
                                            current_local_data, dim=self.input_dim, objective_function=self.obj_func)
        ...
        self.current_iteration += int(x_new.shape[0])
        print("LOCALSEARCH: Completed ", int(x_new.shape[0]), " new turbo evaluations")
        self.global_state.x_global = torch.cat((self.global_state.x_global, x_new), 0)
        self.global_state.y_global = torch.cat((self.global_state.y_global, y_new), 0)
